...
class SupabaseDumpGenerator:
    ...
    def __init__(self, output_dir: str = "database_dumps"):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(self.output_dir / f"dump_generation_{self.timestamp}.log"),
    ...
    async def generate_all_dumps(self, include_data: bool = True, include_schema: bool = True):
        ...
        try:
            ...
            await self.generate_v2_dumps(include_data, include_schema)
    ...
    async def generate_v2_dumps(self, include_data: bool = True, include_schema: bool = True):
        """Generate dumps for V2 database"""
        logger.info("Generating V2 database dumps...")
        
        v2_dir = self.output_dir / "v2"
        v2_dir.mkdir(exist_ok=True)
        await self._generate_pg_dump("v2", V2_DB_CONFIG, v2_dir, include_data, include_schema)
        await self._generate_table_dumps("v2", v2_dir, include_data, include_schema)
        await self._generate_v2_analysis(v2_dir)
    ...
    async def _generate_pg_dump(self, db_version: str, db_config: Dict[str, Any], 
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning("pg_dump not found, skipping pg_dump generation")
                return
            
            timestamp = self.timestamp
            
            if include_schema and include_data:
                dump_file = output_dir / f"{db_version}_complete_dump_{timestamp}.sql"
                cmd = [
                    "pg_dump",
                    "-h", db_config["host"],
                ...
                dump_file = output_dir / f"{db_version}_schema_only_{timestamp}.sql"
                cmd = [
                    "pg_dump",
                    "-h", db_config["host"],
                    "-p", str(db_config["port"]),
                ...
                dump_file = output_dir / f"{db_version}_data_only_{timestamp}.sql"
                cmd = [
                    "pg_dump",
                    "-h", db_config["host"],
                    "-p", str(db_config["port"]),
            ...
            if result.returncode == 0:
                ...
                logger.error(f"pg_dump failed for {db_version}: {result.stderr}")
    ...
    async def _dump_table_schema(self, db_version: str, table_name: str, output_dir: Path):
        ...
        try:
            ...
            schema_file = output_dir / f"schema_{table_name}_{self.timestamp}.sql"
            ...
            with open(schema_file, 'w') as f:
                ...
                for constraint in constraints:
                    if constraint["constraint_type"] == "PRIMARY KEY":
                        f.write(f'ALTER TABLE "{table_name}" ADD CONSTRAINT {constraint["constraint_name"]} PRIMARY KEY ("{constraint["column_name"]}");\n')
                    elif constraint["constraint_type"] == "UNIQUE":
                        f.write(f'ALTER TABLE "{table_name}" ADD CONSTRAINT {constraint["constraint_name"]} UNIQUE ("{constraint["column_name"]}");\n')
            
            logger.info(f"Generated schema dump for table {table_name}: {schema_file}")
    ...
    async def _dump_table_data(self, db_version: str, table_name: str, output_dir: Path):
        ...
        try:
            ...
            logger.info(f"Generated data dump for table {table_name}: {data_file} ({len(data)} rows)")
            ...
            with open(analysis_file, 'w') as f:
                ...
                for table in tables:
                    ...
                    try:
                        ...
                        f.write(f"- **{table_name}**: Error getting count - {e}\n")
                ...
                try:
                    ...
                    if school_stats:
                        ...
                        f.write(f"- Active Schools: {stats['active_schools']}\n")
                        f.write(f"- Deleted Schools: {stats['deleted_schools']}\n")
                except Exception as e:
                ...
                for entity in entities:
                    try:
                        ...
                        if entity_stats:
                            ...
                            f.write(f"### {entity}s\n")
                            f.write(f"- Total: {stats[f'total_{entity.lower()}s']}\n")
                            f.write(f"- Active: {stats[f'active_{entity.lower()}s']}\n")
                            f.write(f"- Deleted: {stats[f'deleted_{entity.lower()}s']}\n\n")
                    except Exception as e:
    ...
    async def generate_schema_comparison(self):
        ...
        try:
            comparison_file = self.output_dir / f"schema_comparison_{self.timestamp}.md"
            ...
            with open(comparison_file, 'w') as f:
                ...
                f.write("- V1 Teacher/Parent/Student → V2 User + role-specific tables\n")
    ...
    async def generate_data_statistics(self):
        ...
        try:
            stats_file = self.output_dir / f"data_statistics_{self.timestamp}.md"
            ...
            with open(stats_file, 'w') as f:
                ...
                f.write("## Summary\n\n")
                f.write("This report provides comprehensive statistics about the data to be migrated.\n\n")
                ...
                try:
                    ...
                    if school_stats:
                        ...
                        f.write(f"- Schools with name: {stats['with_name']} ({stats['with_name']/stats['total']*100:.1f}%)\n")
                        f.write(f"- Schools with email: {stats['with_email']} ({stats['with_email']/stats['total']*100:.1f}%)\n")
                        f.write(f"- Schools with code: {stats['with_code']} ({stats['with_code']/stats['total']*100:.1f}%)\n\n")
                ...
                for entity in entities:
                    try:
                        ...
                        if stats:
                            ...
                            f.write(f"- With school ID: {stat['with_school']} ({stat['with_school']/stat['total']*100:.1f}%)\n")
                            f.write(f"- Valid school ref: {stat['valid_school']} ({stat['valid_school']/stat['total']*100:.1f}%)\n")
                            
                            if stat['total'] > stat['valid_school']:
                                orphaned = stat['total'] - stat['valid_school']
                                f.write(f"- **⚠️ Orphaned records: {orphaned}**\n")
                            f.write("\n")
                            
                    except Exception as e:
                        f.write(f"Error analyzing {entity} relationships: {e}\n\n")
                
                # Data volume estimation
                f.write("## Migration Volume Estimation\n\n")
                f.write("Based on the analysis above, the following data will be migrated:\n\n")
                ...
                f.write("1. **Data Quality**: Review any records missing required fields\n")
                f.write("2. **Orphaned Records**: Fix entity-school relationships before migration\n")
                f.write("3. **Volume Planning**: Prepare for the estimated data volume\n")
                f.write("4. **Backup Strategy**: Ensure comprehensive backups before migration\n")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Generate Supabase database dumps")
    parser.add_argument("--output-dir", default="database_dumps", help="Output directory for dumps")
    parser.add_argument("--no-data", action="store_true", help="Skip data dumps, schema only")
    parser.add_argument("--no-schema", action="store_true", help="Skip schema dumps, data only")
    parser.add_argument("--v1-only", action="store_true", help="Only dump V1 database")
    parser.add_argument("--v2-only", action="store_true", help="Only dump V2 database")
    ...
    try:
        if args.v1_only:
            ...
            await generator.generate_v2_dumps(include_data, include_schema)
        else:
